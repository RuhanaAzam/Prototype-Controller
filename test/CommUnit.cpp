...
void ServerUnit::accept(){
	...
	for(int i = 0;;i++){
		...
		{
			std::cout << "readspeed.txt failed to open\n";
		}
	}
	myfile.close();
...
long ServerUnit::read(int i, Decoder* decoder){
	...
	std::cout << "Received Message Size: " << sizeof(header_) + msgsize << std::endl;
