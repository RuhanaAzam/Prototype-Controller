...
public:
	...
	char *pop(){
		...
		char * retval = inQueue.front();
		inQueue.pop();
		m.unlock();
		return retval;
	}

	StartTransport(char *local1_, char *local2_, char *local3_, char *worker1_, char *worker2_, char *worker3_, char *IP){
...
int main(int argc, char ** argv){
	try{
		...
		StartTransport(local1_, local2_, local3_, worker1_, worker2_, worker3_, IP_);
